...
func makeThread(c config) *shape.Shape {
	switch c.profile {
	case "iso-int":
		return shape.ISOThread(c.radius, c.pitch, false)
	case "acme":
		return shape.AcmeThread(c.radius, c.pitch)
	case "buttress":
		return shape.ANSIButtressThread(c.radius, c.pitch)
	case "plastic-buttress":
		return shape.PlasticButtressThread(c.radius, c.pitch)
	default:
		return shape.ISOThread(c.radius, c.pitch, true)
	}
}
...
func main() {
	...
	for i, c := range configs {
		...
		tris := mesh.CollectTriangles(screw, flrender.NewMarchingCubesOctreeParallel(meshCells))
		be := mesh.CountBoundaryEdges(tris)
	...
	mesh.SaveSTL("screws.stl", allTris)
}
